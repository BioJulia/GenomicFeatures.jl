...
function Base.write{T<:Tuple}(writer::Writer, interval::GenomicFeatures.Interval{T})
    chromid = writer.chroms[interval.seqname][1]
...
function write_impl(writer::Writer, chromid::UInt32, chromstart::UInt32, chromend::UInt32, optionals::Tuple)
    ...
    if state.started && (chromid != state.chromid || position(state.buffer) + position(state.recordbuffer) > writer.max_buffer_size)
        finish_section!(writer)
    end
    if !state.started
